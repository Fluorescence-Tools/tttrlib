// Cross-language PDA (photon distribution analysis) reference test for the Java
// bindings (PRD-001). Builds a Poisson pF and a two-species model, evaluates, and
// asserts the same S1S2 probability matrix as
// test/python/pda/test_pda_cross_language.py and test/r/test_pda.R.
//
// The 2-D S1S2 matrix is read through the get_S1S2_matrix_into(double[]) accessor
// (helpers.i), since the native get_S1S2_matrix uses output pointers.

import io.github.fluorescencetools.tttrlib.Pda;
...
public class PdaTest {
    ...
    public static void main(String[] args) {
        ...
        check(n == REF_N, "S1S2 cells == " + REF_N + " (got " + n + ")");
        check(Math.abs(sum - REF_S1S2_SUM) < 1e-5, "S1S2 sum == " + REF_S1S2_SUM + " (got " + sum + ")");
        check(Math.abs(max - REF_S1S2_MAX) < 1e-6, "S1S2 max == " + REF_S1S2_MAX + " (got " + max + ")");
        ...
        check(Math.abs(hsum - REF_HIST1D_SUM) < 1e-5, "1d histogram sum == " + REF_HIST1D_SUM + " (got " + hsum + ")");
