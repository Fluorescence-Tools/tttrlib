#!/usr/bin/env python3
"""
Direct test for DecayFitXX JSON functionality using the local build
"""
import os
import sys
import json
...
try:
    # Import the local tttrlib module
    import tttrlib
    print("✅ tttrlib imported successfully from local build")
    ...
    json_str = tttrlib.DecayFit.model_to_json_string(model, len(model))
    print("Model JSON:", json_str)
    
    # Parse and verify
    json_data = json.loads(json_str)
    print(f"✅ Model count: {len(json_data['model'])}")
    print(f"✅ First element: {json_data['model'][0]} (expected: 1.1)")
