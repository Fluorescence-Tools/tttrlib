...
H2mmChannelMap H2MM::build_channel_map(
    ...
    // Dense, step 1, ascending: the mapping is a published lookup table, not an
    // arithmetic stride a reader has to reverse-engineer.
    std::vector<int> free_ids;
    for (int id = 0; id <= max_channel; ++id)
        if (used.find(id) == used.end()) free_ids.push_back(id);

    const size_t need = static_cast<size_t>(n_streams_) * n_states;
    if (free_ids.size() < need) {
        ...
        std::string msg =
            "H2MM::build_channel_map: " + std::to_string(n_streams_) + " streams x " +
            std::to_string(n_states) + " states needs " + std::to_string(need) +
            " routing-channel ids, but only " + std::to_string(free_ids.size()) +
            " of 0.." + std::to_string(max_channel) + " are free (" +
            std::to_string(used.size()) + " already in use). The target container's "
            "record field is the limit -- PTU HydraHarp T2/T3 store 6 bits, so ids "
            "above " + std::to_string(H2MM_PTU_MAX_CHANNEL) + " cannot be written "
            "at all, and narrower containers (PicoHarp / SPC-130: 4 bits, "
            "SPC-600/256: 3 bits) truncate silently. "
...
std::shared_ptr<TTTR> H2MM::split_routing_channels(
    ...
    for (int i = 0; i < n_path; ++i) {
        ...
        if (ch < 0) continue;
        // Photons no decoder assigned are simply not visited, so they keep their
        // original id.  After a split the original channels therefore hold only
        // unassigned photons -- summing "channel 0" gives background, not total.
