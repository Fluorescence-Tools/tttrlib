...
void TTTR::bh_fix_missing_markers() {
    // BH SPC-130 marker channels
    const signed char FRAME_CH = 4;
    const signed char LINE_CH = 2;
    const signed char PIXEL_CH = 1;
    const signed char MARKER_TYPE = 1;
    const double THRESHOLD = 0.99;

    if (n_valid_events < 1000) return;

    // === Collect all marker positions ===
    std::vector<size_t> frame_pos, line_pos, pixel_pos;
    for (size_t i = 0; i < n_valid_events; i++) {
        if (event_types[i] == MARKER_TYPE) {
            signed char ch = routing_channels[i];
            if (ch == FRAME_CH) frame_pos.push_back(i);
            else if (ch == LINE_CH) line_pos.push_back(i);
            else if (ch == PIXEL_CH) pixel_pos.push_back(i);
        }
    }

    // === 1. Determine Expected Counts ===
    int n_lines = 0;
    std::string source = "fallback";

    // First try: Get n_lines from header
    if (header != nullptr) {
        try {
            auto tag = header->get_tag(header->json_data, "ImgHdr_PixY");
            if (!tag.is_null() && tag.contains("value")) {
                n_lines = tag["value"];
                source = "header";
            }
        } catch (...) {}
    ...
    // Fallback: Count line markers between last two frame markers
    if (n_lines <= 0) {
        if (frame_pos.size() >= 2) {
            size_t start = frame_pos[frame_pos.size() - 2];
            size_t end = frame_pos.back();
            int marker_count = 0;
            for (size_t lp : line_pos) {
                if (lp > start && lp < end) {
                    marker_count++;
                }
            }
            if (marker_count > 0) {
                n_lines = marker_count - 1; // markers denote edges
            }
        }
    }

    // Abort if < 2 frame markers AND no header value
    if (n_lines <= 0) return;

    int expected_line_markers_per_frame = n_lines + 1;

    // expected_pixels_per_line: Count pixel markers in the last complete line
    // (between the last two line markers before the orphan region)
    int expected_pixels_per_line = 0;
    if (!frame_pos.empty()) {
        size_t orphan_start = frame_pos.back();
        int l1_idx = -1;
        int l2_idx = -1;
        for (int i = (int)line_pos.size() - 1; i >= 0; i--) {
            if (line_pos[i] < orphan_start) {
                if (l1_idx == -1) l1_idx = (int)line_pos[i];
                else {
                    l2_idx = (int)line_pos[i];
                    break;
                }
            }
        }
        if (l1_idx != -1 && l2_idx != -1) {
            for (size_t pp : pixel_pos) {
                if (pp > (size_t)l2_idx && pp < (size_t)l1_idx) {
                    expected_pixels_per_line++;
                }
            }
        }
    }
    
    if (expected_pixels_per_line <= 0) return;

    int expected_pixel_markers_per_frame = expected_pixels_per_line * n_lines;

    // === 2. Determine Timing Intervals ===
    unsigned long long pixel_interval = 1;
    if (pixel_pos.size() >= 2) {
        pixel_interval = get_macro_time_at(pixel_pos.back()) - get_macro_time_at(pixel_pos[pixel_pos.size() - 2]);
    }
    if (pixel_interval == 0) pixel_interval = 1;

    unsigned long long line_interval = 1;
    if (line_pos.size() >= 2) {
        line_interval = get_macro_time_at(line_pos.back()) - get_macro_time_at(line_pos[line_pos.size() - 2]);
    }
    if (line_interval == 0) line_interval = pixel_interval * expected_pixels_per_line;

    // === 3. Detect Orphan Data ===
    if (frame_pos.empty()) return;
    size_t last_frame_marker_pos = frame_pos.back();
    int orphan_pixels = 0;
    for (size_t pp : pixel_pos) {
        if (pp > last_frame_marker_pos) orphan_pixels++;
    }

    if (is_verbose()) {
        std::clog << "-- BH: Expected pixels/line: " << expected_pixels_per_line 
                  << ", lines/frame: " << n_lines 
                  << " (source: " << source << ")" << std::endl;
        std::clog << "-- BH: Orphan pixels: " << orphan_pixels 
                  << ", threshold: " << (int)(THRESHOLD * expected_pixel_markers_per_frame) << std::endl;
    }

    if (orphan_pixels < THRESHOLD * expected_pixel_markers_per_frame) return;

    // === 4. Append Synthetic Markers ===
    
    // 4a: Pad Pixel Markers (for entire frame)
    unsigned long long last_pixel_time = 0;
    if (!pixel_pos.empty()) {
        last_pixel_time = get_macro_time_at(pixel_pos.back());
    } else {
        last_pixel_time = get_macro_time_at(last_frame_marker_pos);
    }

    int added_pixels = 0;
    while (orphan_pixels < expected_pixel_markers_per_frame) {
        unsigned long long new_time = last_pixel_time + pixel_interval;
        append_marker(PIXEL_CH, new_time);
        last_pixel_time = new_time;
        orphan_pixels++;
        added_pixels++;
    }
    ...
    int orphan_lines = 0;
    for (size_t lp : line_pos) {
        if (lp > last_frame_marker_pos) orphan_lines++;
    }
    
    unsigned long long last_line_time = 0;
    if (!line_pos.empty()) {
        last_line_time = get_macro_time_at(line_pos.back());
    } else {
        last_line_time = get_macro_time_at(last_frame_marker_pos);
    }

    int added_lines = 0;
    while (orphan_lines < expected_line_markers_per_frame) {
        unsigned long long new_time = last_line_time + line_interval;
        append_marker(LINE_CH, new_time);
        last_line_time = new_time;
        orphan_lines++;
        added_lines++;
    }

    // 4c: Append Frame Marker
    unsigned long long frame_time = last_line_time + pixel_interval;
    append_marker(FRAME_CH, frame_time);

    if (is_verbose()) {
        std::clog << "-- BH: Appended " << added_pixels << " synthetic pixel markers" << std::endl;
        std::clog << "-- BH: Appended " << added_lines << " synthetic line markers" << std::endl;
        std::clog << "-- BH: Appended 1 synthetic frame marker at time " << frame_time << std::endl;
    }
}

void TTTR::append_marker(int channel, unsigned long long macro_time) {
    size_t new_size = n_valid_events + 1;
    
    // Ensure we have capacity for one more event
    reallocate_memory_for_records(new_size, false);
    
    // Set marker event at end
    set_macro_time_at(n_valid_events, macro_time);
    micro_times[n_valid_events] = 0;
    routing_channels[n_valid_events] = static_cast<signed char>(channel);
    event_types[n_valid_events] = 1;  // Marker event type
