...
* **Creation**: Added [Test workload tiers and the fast lane](/test-workload-tiers.md).
...
* **Initialization**: Established this bundle. The repository root already held a
  ...
  [Fiji plugin — where to continue](/fiji-plugin-handover.md) and gained the
  frontmatter OKF conformance requires. Its body is unchanged.
* **Creation**: Added [tttrlib for JavaScript (Node.js)](/javascript-binding.md),
...
* **Creation**: Added [SWIG Node-API backend traps](/swig-node-api-traps.md).
...
* **Creation**: Added [shared_ptr for the Node-API backend](/shared-ptr-design.md).
...
* **Creation**: Added [PTU web viewer](/ptu-webapp.md), the reference
  application, with the measured results on three file formats.
* **Update**: Recorded the verified state in
  [What is still open](/open-items.md) after the `std::map` conversion compiled
...
* **Creation**: Added [What is still open](/open-items.md). Test coverage is ~60
