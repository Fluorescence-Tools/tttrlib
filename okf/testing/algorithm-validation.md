...
| CLSMImage.h | `compute_ics` / `get_fcs_image` | numpy FFT correlation; **pysimfcs `autocorr2d`** (J. Unruh, live) | pre-existing; identical (0.0) after its N·mean²−1 normalisation and fftshift on even shapes (its `irfft2` drops a column on odd widths) | PASS |
