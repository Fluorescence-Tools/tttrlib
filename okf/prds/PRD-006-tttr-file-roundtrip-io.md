...
> **PRD #:** 006 · **Status:** Done · **Created:** 2026-07-03 · **Closed:** 2026-08-05 · **Owner:** tpeulen
...
## Closing verification (2026-08-05)

Re-verified after the module refactor, which moved the I/O code into
`modules/io/`. `test/python/tttr/test_TTTR_roundtrip.py` now holds **40 tests,
all passing, none skipped** — grown from the 21 at implementation time by the
write-type-inference and BH `.set` sidecar cases that landed later. Coverage is
complete on the matrix: same-format round-trips for CZ-RAW, HT3 (plain v1,
SF-compressed, and v1-header-with-SF-records), Photon-HDF5, PTU HH-T2, PTU
HH-T3, SM, SPC-130, SPC-600_256, synthetic SPC-600_4096; ten transcodes; the
`write()`-returns-`False` failure path. `doc/file-formats.rst` is wired into
both `doc/index.rst` and `doc/user_guide.rst`, and
`examples/tttr/plot_tttr_file_conversion.py` is in the gallery.

One gap closed while verifying: `pq/ht3/pq_ht3v1.0_hh_t3.ht3` and
`pq/ht3/pq_ht3_sf-compression.ht3` had `settings.json` *keys* but no hash and no
`required_files` entry, so `download_test_data.py` never fetched them and the
four SF/v1 tests silently skipped on a fresh checkout. Both are on the download
mirror and byte-identical to the local copies (verified by sha256); they are now
registered, taking the required set to 31 files, all hashed.
