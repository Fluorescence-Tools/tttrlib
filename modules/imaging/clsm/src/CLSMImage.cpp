...
void CLSMImage::ensure_moment_cache(
    ...
    if (!pixels_materialized_ && !stream_masks_.empty()) {
        ...
        if (!stack_frames) {
            ...
            // stacked: run serial and accumulate directly into the plane-sized
            // output. Avoids allocating n_frames*plane per-frame buffers
            // (~41 MB for a 40-frame 256x256 HT3) that the parallel+reduce
            // path needed.
            for_each_mask_photon(false,
                [&](int, size_t l, CLSMLine*, int p, int i) {
                    if (l >= n_lines || static_cast<size_t>(p) >= n_pixel) return;
                    size_t idx = l * n_pixel + p;
                    m0_raw[idx] += 1;
                    m1_raw[idx] += mts[i];
                });
        }
...
void CLSMImage::get_roi(
    ...
    if (subtract_average == "stack") {
        ...
        for (int f = 0; f < nframes_roi; f++) {
            ...
            double mean_count = total_count / pixel_in_roi;
            for (int p = 0; p < pixel_in_roi; p++) {
