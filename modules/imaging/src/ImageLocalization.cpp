...
// autodiff's Dual can carry an Eigen array as its derivative part, so a single
...
// NumericType. Guarded by a unit test that compares the vectorized gradient
// against scalar `dual`, so an autodiff bump fails loudly rather than silently
...
double gauss_gradient(double* u, double* grad_out, void* p) {
    using Arr = Eigen::Array<double, kNModelPar, 1>;
