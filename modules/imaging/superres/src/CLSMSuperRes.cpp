...
#include "Mat.h"         // simd_scale, simd_add — shared SIMD utilities
...
static void scale_inplace_avx(double* out, size_t n, double s) {
    // delegates to shared simd_scale from Mat.h (NEON/SSE2/AVX)
    tttrlib::simd_scale(out, static_cast<int>((n / 4) * 4), s);
    for (size_t i = (n / 4) * 4; i < n; ++i) out[i] *= s;
}

// acc[i] += b[i]
TTTRLIB_TARGET_AVX
static void add_inplace_avx(double* acc, const double* b, size_t n) {
    // delegates to shared simd_add from Mat.h (NEON/SSE2/AVX)
    tttrlib::simd_add(acc, b, static_cast<int>((n / 4) * 4));
    for (size_t i = (n / 4) * 4; i < n; ++i) acc[i] += b[i];
}
