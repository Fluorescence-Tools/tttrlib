...
// row_normalize now lives in Mat.h (tttrlib::row_normalize)
...
// mat_mul / mat_pow now live in Mat.h (tttrlib::mat_power uses SIMD GEMM)
std::vector<double> mat_pow(const std::vector<double>& A, long long e, int n) {
    return mat_power(A.data(), n, static_cast<int>(e));
}
