...
public:
    ...
    /// numpy-buffer overload: takes raw array pointers so Python can pass
    /// NumPy arrays directly (zero-copy) instead of boxing every element
    /// through a std::vector conversion. Output layout is the same as
    /// fit_batch_flat.
    static std::vector<double> fit_batch_flat_buffers(
