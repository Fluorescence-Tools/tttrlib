...
// invert_in_place is now tttrlib::mat_inverse_inplace in Mat.h
...
std::vector<long long> burst_search_kalman(
    ...
    // Hoisted out of the per-bin loop: n_bins is in the millions on a real
    // measurement, and a fresh S_inv plus the inverse's own scratch would be
    ...
    for (int64_t b = 0; b < n_bins; ++b) {
        ...
        S_inv = S;
        if (!mat_inverse_inplace(S_inv.data(), dim, inv_scratch.data())) {
