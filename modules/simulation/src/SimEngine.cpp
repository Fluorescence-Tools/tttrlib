...
SimEngine* SimEngine::from_json(const std::string& json_config) {
    ...
    // A background without a decay writes every background photon into
    // micro-time channel 0 -- a spike at zero is what SCATTER looks like, where
    // uncorrelated background is flat over the laser period. A lifetime fit on
    // such a file fits a large fake scatter component, so a config that means
    // it has to say it.
    if (cfg.contains("background") && !cfg.contains("background_decay")) {
        const auto bg = cfg["background"].get<std::vector<double>>();
        bool any = false;
        for (double b : bg) any = any || b > 0.0;
        if (any)
            std::clog << "WARNING: 'background' is set but 'background_decay' "
                         "is not: every background photon lands in micro-time "
                         "channel 0, which a lifetime fit reads as scatter. "
                         "Declare {\"background_decay\": {\"pattern\": [1.0, "
                         "...], \"dt\": ...}} for the flat micro-time "
                         "distribution uncorrelated background physically has."
                      << std::endl;
    }
    if (cfg.contains("box"))
