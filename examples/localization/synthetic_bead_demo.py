...
This script demonstrates the Image Localization functionality in tttrlib
by creating synthetic fluorescent beads and localizing them using 2D Gaussian fitting.

This serves as a complete demonstration of the enabled Image Localization features.
"""
...
def create_synthetic_beads(image_size=128, num_beads=8, bead_brightness=1000, noise_level=50):
    """
    ...
    Args:
        ...
        bead_brightness (float): Peak intensity of beads
        noise_level (float): Background noise level
        
    Returns:
        tuple: (image, true_positions) where image is 2D numpy array and 
               true_positions is list of (x, y, sigma, amplitude) tuples
    """
    ...
    for i, (x, y, s, a) in enumerate(true_positions):
        print(f"  Bead {i+1}: ({x:.1f}, {y:.1f}), σ={s:.1f}, A={a:.0f}")
    
    return image.astype(float), true_positions

def detect_peaks_threshold(image, threshold_factor=4.0, min_distance=8):
    """
    Simple peak detection using threshold and local maxima
    
    Args:
        image (np.array): 2D intensity image
...
def localize_beads(image, peak_positions, fit_size=15):
    ...
    Args:
        ...
        fit_size (int): Size of fitting region around each peak
        
    Returns:
        list: List of fitted bead parameters
    """
    ...
    for i, (peak_x, peak_y) in enumerate(peak_positions):
        ...
        try:
            ...
            if result > 0:  # Successful fit
                ...
                print(f"    ✓ Fitted: ({fitted_x:.2f}, {fitted_y:.2f}), σ={sigma:.2f}, A={amplitude:.0f}")
            else:
                print(f"    ✗ Fit failed")
...
def visualize_results(image, true_positions, detected_peaks, localized_beads):
    """
    Create visualization of the localization results
    """
    ...
    plt.tight_layout()
    plt.savefig('bead_localization_results.png', dpi=150, bbox_inches='tight')
    print("✓ Saved visualization as 'bead_localization_results.png'")
    plt.show()

def calculate_accuracy_metrics(true_positions, localized_beads):
    """
...
def main():
    ...
    """
    print("="*60)
    print("tttrlib Image Localization Demo")
    print("="*60)
    print("This demo showcases the enabled Image Localization functionality")
    print("by creating synthetic fluorescent beads and localizing them")
    print("using 2D Gaussian fitting with tttrlib.")
    print("="*60)
    ...
    try:
        ...
        # Step 3: Localize beads using tttrlib
        localized_beads = localize_beads(image, detected_peaks, fit_size=15)
        
        # Step 4: Analyze results
        calculate_accuracy_metrics(true_positions, localized_beads)
        
        # Step 5: Visualize results
        visualize_results(image, true_positions, detected_peaks, localized_beads)
        
        print("\n" + "="*60)
        print("✓ Image Localization Demo completed successfully!")
