...
if len(burst_properties) > 0:
    ...
    print(f"Acquisition time: {combined_data.acquisition_time:.1f} seconds")
    print(f"Initial bursts detected: {len(bursts)}")
    print(f"Final bursts after filtering: {len(merged_bursts)}")
    print(f"Burst rate: {len(merged_bursts)/combined_data.acquisition_time:.2f} bursts/second")
